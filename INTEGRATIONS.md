...
### Зафиксированный MVP (обмен и автоматизация)

| Направление | MVP |
|-------------|-----|
| **Банк** | Импорт XML/XLSX; правила сопоставления (`GET/POST /api/bank-match-rules`) подставляют **счёт учёта** и **объект** при импорте, если в назначении или контрагенте есть заданная подстрока. |
| **1С** | Вариант A: периодическая выгрузка **управленческих отчётов** (Excel/PDF) из раздела «Отчёты» для ручного ввода в 1С. Вариант B (следующий шаг): обмен XML/EnterpriseData или публикация HTTP-сервиса 1С — см. раздел «Интеграция с 1С» ниже. |
...
### Импорт выписки (XML и Excel) — реализовано

В системе реализована подгрузка банковской выписки в форматах **XML** и **Excel** (.xlsx, .xls). При импорте **обязательно** указывается банковский счёт — все загруженные операции привязываются к выбранному счёту.

#### Как это работает

1. В разделе «Деньги» добавьте банковские счета: **Импорт выписки** → «Добавить банковский счёт» (название и номер счёта).
2. Выберите счёт из списка и загрузите файл выписки (.xml или .xlsx/.xls).
3. Система разбирает файл и создаёт транзакции с привязкой к выбранному счёту.
4. В разделе «Финансы и платежи» можно фильтровать транзакции по банковскому счёту и видеть колонку «Счёт».

#### API

- **POST /api/integrations/bank/upload** — загрузка выписки (multipart: поле `file`, поле `bank_account_id` обязательно).
- **GET /api/bank-accounts**, **POST /api/bank-accounts** — список и создание банковских счетов.

#### Формат Excel

Поддерживаются заголовки (рус. / англ.): **Дата**, **Сумма** (положительная — приход, отрицательная — расход), **Назначение/Описание**, **Контрагент**. Допускаются варианты: «Дата операции», «Сумма операции», «Назначение платежа», «Плательщик» и т.п. Первый лист книги обрабатывается автоматически.

Пример:

| Дата       | Сумма     | Назначение      | Контрагент   |
|-----------|-----------|-----------------|--------------|
| 2026-02-15 | 50000.00  | Арендная плата  | ООО "Арендатор 1" |
| 2026-02-16 | -5000.00  | Коммунальные    | ООО "УК"     |

#### Формат XML

Поддерживаются структуры с массивом операций и полями даты, суммы, назначения, контрагента. Примеры имён элементов: `operations`, `Operations`, `operation`, `document`, `data`, `выписка`, `операции`. В каждой операции ожидаются поля (в любом регистре/вложенности): дата (`date`, `Date`, `дата`), сумма (`amount`, `Amount`, `Sum`, `Summa`), назначение (`description`, `Purpose`, `Назначение`), контрагент (`counterparty`, `Counterparty`, `Payer`, `Контрагент`). Поддерживается формат 1С-подобный: `Document` → `Operations` → `Operation` с полями `Date`, `Amount`, `Purpose`, `Counterparty`.

Пример упрощённого XML:
...
<Statement>
  <operations>
    <operation>
      ...
      <description>Арендная плата</description>
      <counterparty>ООО "Арендатор 1"</counterparty>
    </operation>
    <operation>
      <date>2026-02-16</date>
      <amount>-5000.00</amount>
      <description>Коммунальные услуги</description>
      <counterparty>ООО "УК"</counterparty>
    </operation>
  </operations>
</Statement>
```
...
**Альтернативный способ (CSV — при необходимости).**
